<div id="styling" class="relative bg-white border-gray-200 rounded-lg shadow dark:border dark:border-gray-700 dark:bg-gray-800">
    ...
    <div>
            ...
            <div class="relative mt-1 rounded-md shadow-sm">
                ...
                <input type="text" wire:model="iconColorSearch" name="icon-color-search" id="icon-color-search" placeholder="Search Colors" class="block w-full pl-10 border-gray-300 rounded-md focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm dark:bg-gray-900 dark:text-gray-200 dark:focus:bg-gray-800 dark:border-gray-700">
            </div>
        </div>
                ...
                @foreach($iconColors as $group => $colors)
                    ...
                    <div class="flex flex-wrap">
                        ...
                        <label for="icon-color-{{ $color }}" class="flex items-center justify-center w-12 h-12 m-1 border border-gray-200 rounded-md dark:border-gray-800" style="background: {{ $color }}">
                            <input id="icon-color-{{ $color }}" type="radio" wire:model="form.styling.color" value="{{ $color }}" class="w-4 h-4 border-gray-300 focus:ring-cyan-500 text-cyan-600">
                        </label>
                        @endforeach
                    ...
                    <label for="icon-color-{{ $colors }}" class="flex items-center justify-center w-12 h-12 m-1 border border-gray-200 rounded-md dark:border-gray-800" style="background: {{ $colors }}">
                        <input id="icon-color-{{ $colors }}" type="radio" wire:model="form.styling.color" value="{{ $colors }}" class="w-4 h-4 border-gray-300 focus:ring-cyan-500 text-cyan-600">
                    </label>
                    @endif
                @endforeach
            </div>
        ...
        <div class="space-y-4">
                ...
                <fieldset class="mt-4">
                    ...
                    <div class="space-y-4 sm:flex sm:items-center sm:space-y-0 sm:space-x-10">
                        <div class="flex items-center">
                            <input id="builder" wire:model="form.styling.type" name="shadow-type" value="builder" type="radio" checked class="w-4 h-4 border-gray-300 focus:ring-cyan-500 text-cyan-600">
                        ...
                        <div class="flex items-center">
                            <input id="manual" wire:model="form.styling.type" name="shadow-type" value="manual" type="radio" class="w-4 h-4 border-gray-300 focus:ring-cyan-500 text-cyan-600">
            ...
            <div>
                ...
                <textarea name="shadow" id="shadow" wire:model="form.styling.shadow" placeholder="filter: drop-shadow(0 0 0.75rem crimson);" class="block w-full border-gray-300 rounded-md focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm dark:bg-gray-900 dark:text-gray-200 dark:focus:bg-gray-800 dark:border-gray-700" rows="10"></textarea>
            </div>
            @endif
        </div>
    </div>
